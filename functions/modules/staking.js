...
// live-aggregator.js reads from this and mirrors into players/{actor}/runtime/live
...
/**
 * Canonical staking doc:
 *   /staking/{actor} = { mining: {...}, polishing: {...}, gems: {...}, updatedAt }
 */
function refs(actor) {
...
async function validateAssetOwnership(actor, assetIds) {
  ...
  const apiPromises = ATOMIC_APIS.map(async (apiBase, index) => {
    ...
    try {
      ...
      console.log(
        `[Staking] ✅ Fastest response from API ${index + 1}: ${apiBase} (${duration}ms) `
        + `=> ${allOwned ? 'VALID' : 'INVALID'} (${ownedAssets.length}/${assetIds.length} owned)`
      );
  ...
  if (successful) {
    return {
      valid: successful.valid,
      ownedAssets: successful.ownedAssets,
    };
  }
...
function normalizeStakingData(stakingData) {
  const out = stakingData && typeof stakingData === 'object' ? { ...stakingData } : {};

  if (!out.mining || typeof out.mining !== 'object') out.mining = {};
  if (!out.polishing || typeof out.polishing !== 'object') out.polishing = {};
  if (!out.gems || typeof out.gems !== 'object') out.gems = {};

  if (out.mining && typeof out.mining === 'object') {
    Object.values(out.mining).forEach(slotData => normalizeSlotSpeedboost(slotData));
  }
...
async function getStakingData(actor) {
  ...
  const raw = snap.data() || {};
  const norm = normalizeStakingData(raw);
  return norm;
}
...
/**
 * Unstake an asset from a specific slot (transaction on /staking/{actor})
 */
 ...
 * Returns the current staking doc
 */
...
/**
 * Gem boost helper for other modules (trading.js)
 */
async function getGemsBoostForType(actor, gemType) {
  ...
  for (const slotData of Object.values(stakingData.gems)) {
