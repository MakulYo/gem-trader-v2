...
const completeMining = onRequest((req, res) =>
  cors(req, res, async () => {
    ...
    // Rate limiting check
    const rateLimitAllowed = await checkRateLimit(actor, 'completeMining', req)
    if (!rateLimitAllowed) {
      return res.status(429).json({
        error: 'Too many requests. Please wait a moment before claiming rewards again.',
        retryAfter: Math.ceil(RATE_LIMITS.completeMining.windowMs / 1000)
      })
    }
    ...
    try {
      ...
      // reward calculation variables (will be set in transaction)
      let slotMP, amt, key, jobData
      ...
      await db.runTransaction(async (tx) => {
        ...
        // Calculate reward: MP / 20 minimum 1
        slotMP = Number(jobData.slotMiningPower) || 0
        amt = Math.max(1, Math.floor(slotMP / 20))
        key = 'rough_gems'
        ...
        // Move job to history and mark as completed
